...
$$h(x) ≝ \sign\left(\sqrt{|x| + 1} - 1\right) + εx$$
with $ε = 10^{-2}$. The additive regularization term ensures that
$h^{-1}$ is Lipschitz continuous.

~~~
It is straightforward to verify that
$$h^{-1} = \sign(x)\left(\left(\frac{\sqrt{1 + 4ε (|x| + 1 + ε)} - 1}{2ε} \right)^2 - 1\right).$$
...
~~~
Note that in Atari, rewards are scaled by $\sign\big(\sqrt{|x| + 1} - 1\big) + εx$ for $ε=10^{-3}$,
