...
entropy objective
$$π_* = \argmax_π 𝔼_{s∼μ_π, a∼π(s)} \big[r(s, a) + α H(π(⋅|s))\big].$$
...
$$\argmax_ν 𝔼_{s, a} \big[q_π(s, a) ν(a | s)\big],$$
...
$$\argmax_ν 𝔼_{s,a} \big[q_π(s, a) ν(a | s) - α \log ν(a|s)\big]
  = \argmax_ν 𝔼_s \Big[𝔼_a \big[q_π(s, a)\big] + αH(ν(⋅|s))\Big].$$

~~~
To maximize it for a given $s$ (assuming finite number of actions for
simplicity), we form a Lagrangian
