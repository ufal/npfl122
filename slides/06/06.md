...
$$→w^* = \argmax_{→w} 𝔼_{τ ∼ π_{→w}} \big[G(τ)\big] = \argmax_{→w} ∫_τ π(τ) G(τ) \d τ,$$
~~~
and the REINFORCE algorithm at each step sets the weights $→w_{t+1}$ to
~~~
$$→w_t + α𝔼_{τ ∼ π_{→w_t}}\bigg[G(τ) \frac{∂\log π_{→w}(τ)}{∂→w}\Big|_{→w=→w_t} \bigg]
= →w_t + α∫_τ π_{→w_t}(τ) G(τ) \frac{∂\log π_{→w}(τ)}{∂→w}\Big|_{→w=→w_t} \d τ.$$
...
$$𝓘 π(τ) ∝ G(τ) π(τ),$$
~~~
$$𝓟 ν = \argmin_{π_{→w}} D_\textrm{KL}\big(ν \| π_{→w}\big).$$
...
~~~
$$D_\textrm{KL}\big(ν \| π_{→w}) = ∫_τ ν(τ) \log \frac{ν(τ)}{π_{→w}(τ)} \d τ.$$

~~~
Therefore, the gradient is
$$\frac{∂ D_\textrm{KL}\big(ν \| π_{→w})}{∂ →w}
 = - ∫_τ ν(τ) ∇_{→w} \log π_{→w}(τ) \d τ
 ∝ - ∫_τ π_{→w}(τ) G(τ) ∇_{→w} \log π_{→w}(τ) \d τ.$$
...
~~~
$$𝓟 ν = \argmin_{π_{→w}} 𝔼_{s∼ν(s)} \Big[D_\textrm{KL}\big(ν(⋅|s) \| π_{→w}(⋅|s)\big)\Big].$$
