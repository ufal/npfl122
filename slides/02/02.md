...
$$\begin{aligned}
v_0(s) &← \begin{cases} 0&\textrm{for the initial state $s$} \\ -∞&\textrm{otherwise} \end{cases} \\
...
Noet that the Bellman equation in current settings has a unique solution if
there are no positive cycles.
In such case, not only does the optimal value function satisfy the Bellman
equation, but the converse statement is also true: If a value function satisfies
...
$$\max_s \big|Bv_1(s) - Bv_2(s)\big| ≤ γ \max_s \big|v_1(s) - v_2(s)\big|.$$
...
![w=95%,h=center](value_iteration_example.svgz)
...
- under the same assumptions as before ($γ<1$ or termination without positive
  cycles), the $v_π$ exists and is unique;
~~~
...
~~~
- given that the Bellman equation for $v_π$ is in fact a system of linear
  equations, apart from iterative solution, $v_π(s)$ can be computed exactly.
