...
class: title
# Markov Decision Process, Optimal Solutions
...
~~~
## Existence
Under some mild assumptions, there always a unique optimal state-value function,
...
An approach devised by Richard Bellman in 1950s.
...
the value function in place.

~~~
Even such asynchronous update it can be proved to
converge, and usually performs better in practise.
...
$$\begin{aligned}
||Bv_1 - Bv_2||_∞ &= ||\max_a 𝔼\left[R_{t+1} + γ v_1(S_{t+1})\right] - \max_a 𝔼\left[R_{t+1} + γ v_1(S_{t+1})\right]||_∞ \\
                  &≤ \max_a\left( || 𝔼\left[R_{t+1} + γ v_1(S_{t+1})\right] - 𝔼\left[R_{t+1} + γ v_1(S_{t+1})\right]||_∞\right) \\
                  &= \max_a\left( \left|\left| ∑\nolimits_{S_{t+1},R_{t+1}} p\left(S_{t+1}, R_{t+1} \middle| s, a\right)γ(v_1(S_{t+1}) - v_2(S_{t+1}))\right|\right|_∞\right) \\
                  &= γ \max_a\left(\left|\left| ∑\nolimits_{S_{t+1}} p\left(S_{t+1} \middle| s, a\right)(v_1(S_{t+1}) - v_2(S_{t+1}))\right|\right|_∞\right) \\
                  &≤ γ ||v_1 - v_2||_∞,
\end{aligned}$$
...
$∑_{S_{t+1}} p(S_{t+1} | s, a)$ sums to 1.
...
One of the simplest way to achieve that is to assume _exploding starts_, where
...
---
# Monte Carlo with Exploding Starts

![w=90%,h=center](monte_carlo_exploding_starts.pdf)
