...
$$v_π(s) = 𝔼_π[q_π(s, a)],~~~~~~~q_π(s, a) = 𝔼[R_{t+1} + γv_π(S_{t+1}) | S_t = s, A_t = a].$$
...
$$\sup_s \big|Bv_1(s) - Bv_2(s)\big| ≤ γ \sup_s \big|v_1(s) - v_2(s)\big|.$$

~~~
Considering a normed vector space $ℝ^{|𝓢|}$ with supremum norm $||⋅||_∞$,
from the Banach fixed-point theorem it follows there exists a _unique value function_
$v_*$ such that $Bv_* = v_*$.
...
||Bv_1 - Bv_2||_∞ &= ||\max_a 𝔼\left[R_{t+1} + γ v_1(S_{t+1})\right] - \max_a 𝔼\left[R_{t+1} + γ v_2(S_{t+1})\right]||_∞ \\
                  &= ||\max_a \big(𝔼\left[R_{t+1} + γ v_1(S_{t+1})\right] - \max_a 𝔼\left[R_{t+1} + γ v_2(S_{t+1})\right]\big)||_∞ \\
                  &≤ ||\max_a \big(𝔼\left[R_{t+1} + γ v_1(S_{t+1})\right] - 𝔼\left[R_{t+1} + γ v_2(S_{t+1})\right]\big)||_∞ \\
                  &= \max_a\big( || 𝔼\left[R_{t+1} + γ v_1(S_{t+1})\right] - 𝔼\left[R_{t+1} + γ v_2(S_{t+1})\right]||_∞\big) \\
