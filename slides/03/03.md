...
We can then estimate $𝔼_{⁇x∼b}\big[f(x)\big]$ as $\frac{1}{N} ∑_i f(x_i).$

~~~
In order to estimate $𝔼_{⁇x∼π}\big[f(x)\big]$ using the samples $x_i$, we need to account
for different probabilities of $x_i$ under the two distributions. It is
straightforward to verify that
$$𝔼_{⁇x∼π}\big[f(x)\big] = 𝔼_{⁇x∼b}\left[\frac{π(x)}{b(x)}f(x)\right].$$

~~~
Therefore, we can estimate 
$$𝔼_{⁇x∼π}\big[f(x)\big] ≈ \frac{1}{N} ∑_i \frac{π(x_i)}{b(x_i)} f(x_i),$$
