...
# Eligibility traces, V-trace, IMPALA, PopArt Normalization
...
G_{t:t+n} - V(S_t)
  &= R_{t+1} + γ G_{t+1:t+n} \\
  &= R_{t+1} + γ \big(G_{t+1:t+n} - V(S_{t+1})\big) + γV(S_{t+1}),
\end{aligned}$$
which yields
$$G_{t:t+n} = R_{t+1} + γV(S_{t+1}) - V(S_t) + γ G_{t+1:t+n}.$$
...
~~~
First note that
$$𝔼_{A_t ∼ μ} \big[ρ_t\big] = ∑_{A_t} μ(A_t | S_t) \frac{π(A_t | S_t)}{b(A_t | S_t)},$$
...
the expectation of the importance sample ration is 1, we can simplify to
...
G_{t:t+n}^\mathrm{CV} - V(S_t)
  ...
  &= ρ_t \big(R_{t+1} + γ (G_{t+1:t+n}^\mathrm{CV} - V(S_{t+1})) + γ V(S_{t+1}) - V(S_t)\big)
\end{aligned}$$
results in
$$G_{t:t+n}^\mathrm{CV}
  = ρ_t \big(R_{t+1} + γV(S_{t+1}) - V(S_t) + γ G_{t+1:t+n}^\mathrm{CV}\big)
...
$$G_{t}^λ ≝ (1 - λ) ∑_{i=1}^∞ λ^{i-1} G_{t:t+i}.$$

~~~
![w=78%,h=center](traces_weighting.svgz)
...
to
$$G_{t}^λ = (1 - λ) ∑_{i=1}^{T-t-1} λ^{i-1} G_{t:t+i} + λ^{T-t-1} G_t.$$
...
**truncated $λ$-return**
$$G_{t:t+n}^λ ≝ (1 - λ) ∑_{i=1}^n λ^{i-1} G_{t:t+i} + λ^n G_{t:t+n}.$$

~~~
Considering that the $n$-step returns can be written as a sum of TD errors
$$G_{t:t+n} = ∑\nolimits_{i=0}^{n-1} γ^i δ_{t+i},$$
we can also obtain the truncated $λ$-return as a combination of TD errors:
$$\begin{aligned}
  G_{t:t+n}^λ
  &= (1-λ)\left(∑\nolimits_{i=1}^n λ^{i-1} δ_t + λ^n δ_t\right) + γλG_{t+1:t+n}^λ \\
  &= δ_t +  γλG_{t+1:t+n}^λD \\
...
step $i$. Then, replacing $λ^n$ by $∏_{i=1}^n λ_{t+i}$ and suitably normalizing,
we obtain

$$G_{t:t+n}^λ = ∑_{i=0}^{n-1} γ^i \left(∏_{j=1}^i λ_{t+j}\right) δ_{t+i}.$$
