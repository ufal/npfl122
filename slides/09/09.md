...
~~~
Therefore, $→d_F$ is found as a solution of $$F(→θ)→d_F = ∇ v_π$$ and using only 10
iterations of the algorithm seem to suffice according to the experiments.
...
~~~
Considering two policies $π, \tilde π$, we can write
$$v_{\tilde π} = v_π + 𝔼_{s ∼ μ(\tilde π)} 𝔼_{a ∼ \tilde π(a | s)} a_π(a | s),$$
where $a_π(a | s)$ is the advantage function $q_π(a | s) - v_π(s)$ and
$μ(\tilde π)$ is the on-policy distribution of the policy $\tilde π$.

~~~
Analogously to policy improvement, we see that if $a_π(a | s) ≥0$, policy
$\tilde π$ performance increases (or stays the same if the advantages are zero
everywhere).

~~~
However, sampling states $s ∼ μ(\tilde π)$ is costly. Therefore, we instead
consider
$$L_π(\tilde π) = v_π + 𝔼_{s ∼ μ(π)} 𝔼_{a ∼ \tilde π(a | s)} a_π(a | s).$$

---
# Trust Region Policy Optimization
$$L_π(\tilde π) = v_π + 𝔼_{s ∼ μ(π)} 𝔼_{a ∼ \tilde π(a | s)} a_π(a | s)$$

It can be shown that for parametrized $π(a | s; →θ)$ the $L_π(\tilde π)$ matches
$v_{\tilde π}$ to the first order.
...
$$L^\textrm{CLIP}(→θ) ≝ 𝔼_t\Big[\min\big(r_t(→θ) \hat A_t, \operatorname{clip}(r_t(→θ), 1-ε, 1+ε) \hat A_t)\big)\Big].$$
...
The advantages $\hat A_t$ are additionally estimated using _generalized
advantage estimation_. Instead of the usual
$\hat A_t ≝ ∑_{i=0}^{T-t-1} γ^i R_{t+1+i} + γ^{T-t} V(S_T) - V(S_t)$
the authors employ
$$\hat A_t ≝ ∑_{i=0}^{T-t-1} (γλ)^i δ_{t+i},$$
