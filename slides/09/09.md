...
$$\begin{aligned}
  ...
  &= \underbrace{∑_a π_ρ̄(a|S_t) \Big[R_{t+1} + γ 𝔼_{s' ∼ p(S_t, a)} V^{π_ρ̄}(s') - V^{π_ρ̄}(S_t)\Big]}_{=0} ∑_{a'} \min\big(ρ̄ b(a'|S_t), π(a'|S_t) \big), \\
\end{aligned}$$
...
The NGU agent uses transformed Retrace loss and considers rewards $r_t^β ≝ r_t^e + β r_t^i$ for

$\displaystyle\kern5em
  r_t^i ≝ r_t^\textrm{episodic} ⋅ \operatorname{clip}\Big(1 ≤ α_t=1 + \tfrac{(ĝ - g)^2 - μ_\textrm{err}}{σ_\textrm{err}} ≤ L=5\Big).$

~~~
We train a parametrized action-value function $q(s, a, β_i)$ corresponding to
reward $r_t^{β_i}$ for $β_0=0$ and $γ_0=0.997, …, β_{N-1}=β$ and $γ_{N-1}=0.99$,
...
~~~
  In fact, the transformed rewards are used as $q(s, a, j; →θ) ≝ h(h^{-1} q(s, a, j; →θ^e) + β_j h^{-1} q(s, a, j; →θ^i))$.
