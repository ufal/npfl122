...
~~~
- mean action value $Q(s, a) ≝ W(s, a) / N(s, a)$, which is usually not stores
  explicitly,
~~~
...
~~~
Also, the reason for the modification of the UCB formula was never discusses in
any of AlphaZero paper and is not obvious.
...
~~~
- add all its children with $N=W=Q=0$ and the prior $→p$,
~~~
- in the backward pass for all $t ≤ L$, we update the statistics in nodes by performing
  $N(s_t, a_t) ← N(s_t, a_t) + 1$ and $W(s_t, a_t) ← W(s_t, a_t) + v$.

~~~
![w=100%,mh=70%,v=middle](ag0_mcts.svgz)
...
~~~
The next move is chose as either
