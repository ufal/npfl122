...
~~~
$\displaystyle \phantom{∇_{→θ} v_π(s)} = ∇_{→θ}\Big(r\big(s, π(s; →θ)\big) + ∫_{s', r} p\big(s', r | s, π(s; →θ)\big)\big(r + γv_π(s')\big) \d s'\Big)$

~~~
$\displaystyle \phantom{∇_{→θ} v_π(s)} = ∇_{→θ} π(s; →θ) ∇_a r(s, a) \big|_{a=π(s; →θ)} + ∇_{→θ} γ∫_{s'} p\big(s' | s, π(s; →θ)\big) v_π(s') \d s'$

~~~
$\displaystyle \phantom{∇_{→θ} v_π(s)} = ∇_{→θ} π(s; →θ) ∇_a \Big(r(s, a) + γ∫_{s'} p\big(s' | s, a\big) v_π(s') \d s' \Big) \Big|_{a=π(s; →θ)}\\
