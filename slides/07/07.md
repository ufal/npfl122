...
theorem, maximizing $J(→θ) ≝ 𝔼_{h, π} v_π(s)$. The objective gradient is defined as
$$\begin{aligned}
  ∇_→θ J(→θ) &∝ ∑_{s∈𝓢} μ(s) ∑_{a ∈ 𝓐} q_π(s, a) ∇_→θ π(a | s; →θ) \\
...
```python
  ...
  sd = tf.math.exp(sd)   # or sd = tf.math.softplus(sd)

  normal_dist = tfp.distributions.Normal(mu, sd)
